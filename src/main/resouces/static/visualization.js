function parseText(text) {
    ...
    json.forEach(function (vechicle) {
        var color1 = 'rgb(' + (Math.floor((256 - 99) * Math.random()) + 100) + ',' +
            (Math.floor((256 - 99) * Math.random()) + 100) + ',' +
            (Math.floor((256 - 99) * Math.random()) + 100) + ')';
        var color2 = 'rgb(' + (Math.floor((256 - 99) * Math.random()) + 100) + ',' +
            (Math.floor((256 - 99) * Math.random()) + 100) + ',' +
            (Math.floor((256 - 99) * Math.random()) + 100) + ')';
        //console.log(color);
        var from = 0;
        vechicle.routes.forEach(function (route) {
            var color = route.type === 'PICKUP' ? color1 : color2;
            //console.log(color)
            dataSet.push({
                id: route.id,
                label: "" + route.id,
                color: color,
                x: route.location.x * scale,
                y: route.location.y * scale,
                title: "volume:" + route.volume + ", sibling:" + route.sibling
            });
